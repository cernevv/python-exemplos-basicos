...
def validate_lenght(input):
    if input.indigit() and len(input) <= 2 and int(input) <= 32:
        return True
    elif input == "":
        return True
    else:
...
validate_command = root.register(validate_lenght)
...
ctk.CTkLabel(lf, text="Quantos cracteres (Máximo 32)?", font=("Helvetica", 16)).pack(pady=10)

# Label para o input do comprimento da senha
ctk.CTkLabel(lf, font=("Helvetica", 24), validate="key", validatecommand=(validate_command, '%P'), width=80, justify='center')
...
#Carrega as imagens para os botôes
try:
    ...
    copy_image = ctk.CtkImage(light_image=Image.open(resource_path("create.png")), dark_image=Image.open(resource_path("copy.png")), size=(20,20))
    clean_image = ctk.CtkImage(light_image=Image.open(resource_path("create.png")), dark_image=Image.open(resource_path("clean.png")), size=(20,20))
except Exception as e:
...
clip_button = ctk.CTkButton(my_frame, text="Copiar", command=clipper, image=copy_image, compounds="left", font=("Helvetica", 16)) 
